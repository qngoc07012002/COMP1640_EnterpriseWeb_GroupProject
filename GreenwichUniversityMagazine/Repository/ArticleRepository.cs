        ...
        public Article GetById(int id)
        {
            ...
            public IEnumerable<int> GetArticleCountsByTerm()
            {
                var query = from term in _dbContext.Terms
                            ...
                            join article in _dbContext.Articles on magazine.Id equals article.MagazinedId into articleGroup
                            group articleGroup by new { term.Name, term.EndDate } into g
                            orderby g.Key.EndDate
                            select g.SelectMany(x => x).Count();
            ...
            public async Task<IEnumerable<object>> GetMultipleLineChartData()
            {
                var query = from trm in _dbContext.Terms
                            ...
                            join art in _dbContext.Articles on mag.Id equals art.MagazinedId into articleGroup
            ...
            public async Task<IEnumerable<object>> GetGroupBarChart()
            {
                var query = from trm in _dbContext.Terms
                            from fac in _dbContext.Faculties
                            join mag in _dbContext.Magazines on new { TermId = trm.Id, FacultyId = fac.Id } equals new { mag.TermId, mag.FacultyId } into magazineGroup
            ...
            public async Task<IEnumerable<object>> GetStackBarChartContributions()
            {
                var query = from trm in _dbContext.Terms
                            from fac in _dbContext.Faculties
                            join mag in _dbContext.Magazines on new { TermId = trm.Id, FacultyId = fac.Id } equals new { mag.TermId, mag.FacultyId } into magazineGroup
