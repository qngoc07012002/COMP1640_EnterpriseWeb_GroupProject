...
namespace GreenwichUniversityMagazine.Areas.Manager.Controllers
{
  ...
  .Select(u => new SelectListItem
  {
      ...
      Value = u.Id.ToString()
        ...
        {
            // Loại bỏ các thẻ HTML và chuyển đổi các ký tự đặc biệt HTML
        ...
        public IActionResult DownloadAllArticles(int selectedMagazineId)
        {
            var articles = _unitOfWork.ArticleRepository.GetAll()
            ...
            using (var archive = ZipFile.Open(zipFilePath, ZipArchiveMode.Create))
            {
                foreach (var article in articles)
                {
                    ...
                    if (Directory.Exists(articleFolderPath))
                    {
                        ...
                        archive.CreateEntry(folderPathInArchive); // Tạo thư mục con cho mỗi bài báo

                        foreach (var filePath in Directory.GetFiles(articleFolderPath))
                        {
                            var entryName = Path.GetFileName(filePath);
                            var entryPathInArchive = folderPathInArchive + entryName;
                            archive.CreateEntryFromFile(filePath, entryPathInArchive); // Tạo các entry trong thư mục con
                        }
                    }

                    var imageUrl = article.imgUrl; // Giả định rằng article.imgUrl là một chuỗi đường dẫn duy nhất
                    var imageName = Path.GetFileName(imageUrl);
                    var imagePhysicalPath = Path.Combine(_hostEnvironment.WebRootPath, imageUrl.TrimStart('/'));
                    if (System.IO.File.Exists(imagePhysicalPath))
                    {
