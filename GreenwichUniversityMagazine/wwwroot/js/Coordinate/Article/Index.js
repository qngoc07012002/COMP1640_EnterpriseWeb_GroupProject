        ...
        articles.forEach(article => {
            ...
            if (new Date(article.magazines.term.startDate) <= new Date() && new Date(article.magazines.term.endDate) >= new Date() && article.status === false) {
                if (new Date(article.magazines.endDate) >= new Date()) {
                    ...
                    articleCard += `<a href="/coordinate/coordinate/Update/id=${article.articleId}" class="btn btn-primary mx-2"> <i class="bi bi-pencil-square"></i> Modify</a>`;
                } else {
                    articleCard += `<a href="/coordinate/coordinate/Update/id=${article.articleId}&status=outMagazine" class="btn btn-primary mx-2"> <i class="bi bi-pencil-square"></i> Modify</a>`;
                }

            } else {
                if (article.status === true) {
                    articleCard += `<a href="/coordinate/coordinate/Update/id=${article.articleId}" class="btn btn-primary mx-2"> <i class="bi bi-pencil-square"></i>View</a>`;
                }
                else {
